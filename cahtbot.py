...
lines = open('movie_lines.txt', encoding = 'utf-8', errors = 'ignore').read().split('\n') #Reading from the file and formatting 
conversations = open('movie_conversations.txt', encoding = 'utf-8', errors = 'ignore').read().split('\n')


id2line={}
for line in lines:
    _line = line.split(' +++$+++ ') # Line splitting in order to take the elements we want
    if len(_line) == 5:             # To maintain all the lines of the same no. of elements
        id2line[_line[0]] = _line[4] #mapping dictionary
...
# Craeting two dictionaries for Q&A; 
threshold = 20
questionswords2int = {}
word_number = 0
...
for word, count in word2count.items():
    if count > threshold:
        ...
        word_number += 1
# Adding tokens for spesification
...
# Converting words in questions to int 
